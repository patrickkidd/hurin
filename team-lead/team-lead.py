...
Architecture:
  ...
  GitHub (PRs, CI, Issues, Project #4) → [team-lead polls every 15min]
    ...
    → Auto-spawn pipeline + Discord #team-lead
"""
...
def load_tokens():
    ...
    # Read Discord token from secrets.json (canonical), fallback to legacy file
    secrets_file = Path.home() / ".openclaw" / "secrets.json"
    if secrets_file.exists():
        try:
            secrets = json.loads(secrets_file.read_text())
            DISCORD_BOT_TOKEN = secrets.get("discord-bot-token", "")
        except (json.JSONDecodeError, KeyError):
            pass
    if not DISCORD_BOT_TOKEN and DISCORD_TOKEN_FILE.exists():
...
def post_to_teamlead(content, session_id="", synthesis_file=""):
    """Post a message to #team-lead as a thread for reply monitoring.
    ...
    thread_id, msg_id = create_channel_thread(
        DISCORD_TEAMLEAD_CHANNEL_ID, first_chunk, thread_name
    )
    ...
    if thread_id:
        ...
        if session_id:
            register_channel_thread(
                ...
                channel_id=DISCORD_TEAMLEAD_CHANNEL_ID,
        ...
        for i, chunk in enumerate(chunks):
            ...
            discord_api(
                "POST",
                f"https://discord.com/api/v10/channels/{DISCORD_TEAMLEAD_CHANNEL_ID}/messages",
                {"content": chunk[:2000]},
            )
...
ANOMALY_COOLDOWNS_FILE = TEAM_LEAD_DIR / "anomaly-cooldowns.json"


def _load_anomaly_cooldowns():
    if not ANOMALY_COOLDOWNS_FILE.exists():
        return {}
    try:
        return json.loads(ANOMALY_COOLDOWNS_FILE.read_text())
    except (json.JSONDecodeError, IOError):
        return {}


def _save_anomaly_cooldowns(cooldowns):
    ANOMALY_COOLDOWNS_FILE.write_text(json.dumps(cooldowns))
...
def detect_anomalies(github_data, metrics, events):
    ...
    now = time.time()
    anomalies = []
    cooldowns = _load_anomaly_cooldowns()

    def _emit(atype, severity, message):
        last = cooldowns.get(atype, 0)
                ...
                if age_hours > STALE_PR_HOURS:
                    ...
                    if all(c == "SUCCESS" for c in conclusions) and conclusions:
                        _emit(
                            ...
                            "medium",
                            f"https://github.com/{pr['repo']}/pull/{pr['number']} PR #{pr['number']} ({pr.get('title', '')}) is stale — "
    ...
    _save_anomaly_cooldowns(cooldowns)
    return anomalies


# ---------------------------------------------------------------------------
# Read recent events from task-events.jsonl
# ---------------------------------------------------------------------------

def read_recent_events(max_events=100):
    """Read recent events from task-events.jsonl (no daemon needed)."""
    if not TASK_EVENTS.exists():
        return []
    events = []
    try:
    ...
    if all_repo_prs:
        ...
        for state_name in ["CHANGES_REQUESTED", "APPROVED", "PENDING", "REVIEW_REQUIRED"]:
            ...
            for pr in prs_in_state:
                ...
                pr_dashboard_lines.append(
                    f"    - https://github.com/{pr['repo']}/pull/{pr['number']} {pr['repo']} PR #{pr['number']}: {pr['title']}"
...
{chr(10).join(f"  - https://github.com/{t.get('repo', '')}/pull/{t.get('pr', '')} PR #{t.get('pr', '')}: {t.get('description', '')}" for t in pr_open) or "  None"}
...
def process_synthesis(synthesis, github_data, session_id=""):
    ...
    post_to_teamlead(message, session_id=session_id, synthesis_file=synthesis_file)
